...
pop = gen_population(N_pop = N_pop, mean_psu_size = 80, K = K, sigma_u = 1.2) # change sigma_u for varying ICC and rerun
...
for (g in groups) {
  ...
  res_tbl_cluster$rel_bias_naive[res_tbl_cluster$group == g] =
    100 * mean(hat_n_g - true_g, na.rm = TRUE) / mean_true
  res_tbl_cluster$rel_bias_wt[res_tbl_cluster$group == g] =
    100 * (mean_w - mean_true) / mean_true
  res_tbl_cluster$rel_bias_swlb[res_tbl_cluster$group == g] =
    100 * (mean(hat_s_g, na.rm = TRUE) - mean_true) / mean_true
  res_tbl_cluster$rel_bias_dowlb[res_tbl_cluster$group == g] =
  ...
  res_tbl_cluster$rel_rmse_wt[res_tbl_cluster$group == g]    = 100 * rmse_w / mean_true
  res_tbl_cluster$rel_rmse_swlb[res_tbl_cluster$group == g]  = 100 * rmse_s / mean_true
  res_tbl_cluster$rel_rmse_dowlb[res_tbl_cluster$group == g] = 100 * rmse_d / mean_true
  ...
  res_tbl_cluster$cov_swlb[res_tbl_cluster$group == g] =
    mean(true_g >= s_Lg & true_g <= s_Ug, na.rm = TRUE)
  res_tbl_cluster$cov_dowlb[res_tbl_cluster$group == g] =
    mean(true_g >= d_Lg & true_g <= d_Ug, na.rm = TRUE)

  res_tbl_cluster$var_ratio_swlb[res_tbl_cluster$group == g] = mean_s_var / var_w
  res_tbl_cluster$var_ratio_dowlb[res_tbl_cluster$group == g] = mean_d_var / var_w

  res_tbl_cluster$bias_wt[res_tbl_cluster$group == g]  = bias_w
  res_tbl_cluster$var_wt[res_tbl_cluster$group == g]   = var_w
  res_tbl_cluster$mse_wt[res_tbl_cluster$group == g]   = mse_w
  if (mse_w > 0) {
    res_tbl_cluster$bias2_over_mse_wt[res_tbl_cluster$group == g] = (bias_w^2) / mse_w
    res_tbl_cluster$var_over_mse_wt[res_tbl_cluster$group == g]   = var_w / mse_w
  }

  res_tbl_cluster$mse_swlb[res_tbl_cluster$group == g]    = mse_s
  res_tbl_cluster$mse_dowlb[res_tbl_cluster$group == g]   = mse_d
  if (mse_w > 0) {
    res_tbl_cluster$mse_ratio_swlb[res_tbl_cluster$group == g]  = mse_s / mse_w
    res_tbl_cluster$mse_ratio_dowlb[res_tbl_cluster$group == g] = mse_d / mse_w
  }
}
```
...
```{r}
############################################################
# 6. Summaries: bias, RMSE, coverage, variance, MSE
############################################################
